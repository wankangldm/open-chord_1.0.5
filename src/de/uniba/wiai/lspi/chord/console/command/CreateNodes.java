...
import de.uniba.wiai.lspi.util.node.NodeTxtLoader;
...
public class CreateNodes extends de.uniba.wiai.lspi.util.console.Command {
     ...
     */
    public static final String NAMES_PARAM = "nodes";
...
//    public static final String EXECUTOR_PARAM = "executor";
    ...
    public void createNodes(List<String> names) throws ConsoleException {
        if ( names.size()   >= 1){
            ...
            if (reg.lookupAll().size() > 0){
                ...
                for (int i = 0; i < names.size(); i++){
                    if (i==0){
                        this.out.println("Starting node with ID '" +names.get(0) + "'");
                        Chord node = null;
                        node = new ChordImpl();
                        try {
    ...
    private void join(String nodeToCreate) throws ConsoleException {
            this.out.println("Starting node with ID '" +nodeToCreate + "'");
            ...
            if (reg.lookup(url) != null){
                ...
                try {
                    ...
                    for (Object in : eps.values()) {
                        ThreadEndpoint ep = (ThreadEndpoint) in;
                        bootstrap = ep.getURL().getHost();
                    }
